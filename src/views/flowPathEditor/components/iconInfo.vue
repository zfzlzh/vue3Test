...
export default defineComponent({
    ...
    setup(props,ctx){
        ...
        const state = reactive({
              formList:[
                {prop:'name',label:'图形名称',type:'input'},
        ...
        watch(() => props.clickShapesItem,(newVal)=>{
            if(newVal.id == state.setForm.id || !newVal.style){
                return
            }
            let style = newVal.style
            state.setForm = Object.keys(state.setForm).reduce((pre:any,item:any)=>{
                pre[item] = style[item] ? style[item] : newVal[item] ? newVal[item] : pre[item]
                if(pre[item] == 'transparent'){
                    pre[item] = ''
                }
                return pre
            },state.setForm as any)
            console.log(state.setForm)
        },{deep:true})
        return {
           ...toRefs(state),
           ...toRefs(methods),
